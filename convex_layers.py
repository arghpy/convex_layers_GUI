...
def convexHull_2(listPoints, n):
    ...
    while True:
        ...
        for i in range(n):
            if orientation(listPoints[p], listPoints[i], listPoints[q]) == 2:
                q = i
...
button1 = Button(
    win,
    text="Compute Convex Layers",
    command=lambda: convexHull_2(listPoints, len(listPoints)),
)
